...
class ModulatedGraphConv(nn.Module):
    ...
    def forward(self, input):
        ...
        # input = input.to('cpu')
        h0 = torch.matmul(input, self.W[0]) #input = [256, 17, 2] W = [2,2,384] depends on input and output features
        h1 = torch.matmul(input, self.W[1])
        
        adj = self.adj.to(input.device) + self.adj2.to(input.device)
        # adj = self.adj.to('cpu') + self.adj2.to('cpu')
        # adj = (adj.T + adj)/2
        adj = (adj.transpose(0, 1) + adj)/2
        # E = torch.eye(17, dtype=torch.float).to(input.device)
        E = torch.eye(adj.size(0), dtype=torch.float).to(input.device)
        # E = torch.eye(adj.size(0), dtype=torch.float).to('cpu')
